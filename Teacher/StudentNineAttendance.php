...
<body>
	...
	<div class="container" style="margin-top: 1px;">
		...
		<div class="panel-body">
			...
			<a href="AttendanceNineView.php" class="btn btn-primary">view</a>&nbsp<?php echo date('d-m-y')?>
	...
	<input type="submit" name="sub" value="Submit" style="text-align: center;" class="btn btn-primary">
	 <a href="StudentAttendance.php" class="btn btn-danger">Cancel
			...
			<?php
				if(isset($_POST['sub']))
				{
					...
					$r = mysqli_query($con,"select distinct Date from attendancenine");
					$b=false;
					...
					if(!$b)
					{
						foreach ($att as $key => $value)
						{
							if ($value == "Present")
							{
								$q = mysqli_query($con,"insert into attendancenine(RollNo,Date,Value) values($key,'$date','Present')");
							}
							else
							{
								$q = mysqli_query($con,"insert into attendancenine(RollNo,Date,Value) values($key,'$date','Absent')");
							}
						}
						if($q > 0)
						{
