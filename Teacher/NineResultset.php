...
$q=mysqli_query($con,"select Subject,Marks,Date from timetablenine where Sno=$id");
...
<html lang="en">
<head>
  <title>New Result Set For 9th Class</title>
  <meta charset="utf-8">
...
foreach ($mar as $key=> $value) 
{
  ...
  $q = mysqli_query($con,"insert into resultnine (Rollno,Name,Subject,Mark,Total,Date_Ex) values ('{$_POST['no'][$key]}','{$_POST['nm'][$key]}','$sb','$value','$tot','$dt')");
...
if($q)
{
  $set=mysqi_query($con,"update timetablenine set isSeted=1 where Sno='$id'");
